...
@section('title', 'Edit Promo - Toko Bunga')
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white border border-gray-200 rounded-lg p-6 mb-6">
                ...
                <h2 class="text-2xl font-bold text-black mt-4">Edit Promo</h2>
                <p class="text-gray-600 mt-1">Perbarui informasi promo atau diskon</p>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-lg p-6">
                <form action="{{ route('admin.promo.update', $promo->id) }}" method="POST">
                    ...
                    <div class="mb-8">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', $promo->name) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-black focus:border-black" placeholder="Contoh: Diskon Lebaran 50%" required>
                            </div>
                            <div>
                                <label for="description" class="block text-sm font-medium text-gray-700 mb-2">Deskripsi</label>
                                <textarea name="description" id="description" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-black focus:border-black" placeholder="Jelaskan detail promo...">{{ old('description', $promo->description) }}</textarea>
                            </div>
                            <div>
                                <label for="code" class="block text-sm font-medium text-gray-700 mb-2">Kode Promo (Opsional)</label>
                                <input type="text" name="code" id="code" value="{{ old('code', $promo->code) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-black focus:border-black" placeholder="Kosongkan untuk generate otomatis">
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="number" name="discount_value" id="discount_value" value="{{ old('discount_value', $promo->discount_value) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-black focus:border-black" placeholder="50 atau 50000" step="0.01" required>
                                <p class="text-sm text-gray-500 mt-1" id="discount_hint">
                                    @if(old('discount_type', $promo->discount_type) == 'percentage')
                                        Masukkan persentase (contoh: 50 untuk 50%)
                                    @else
                                        Masukkan nominal dalam rupiah (contoh: 50000)
                                    @endif
                                </p>
                            </div>
                            <div>
                                <label for="min_purchase" class="block text-sm font-medium text-gray-700 mb-2">Minimum Pembelian (Opsional)</label>
                                <input type="number" name="min_purchase" id="min_purchase" value="{{ old('min_purchase', $promo->min_purchase) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-black focus:border-black" placeholder="100000" step="0.01">
                            </div>
                            <div>
                                <label for="max_discount" class="block text-sm font-medium text-gray-700 mb-2">Maksimal Diskon (Opsional)</label>
                                <input type="number" name="max_discount" id="max_discount" value="{{ old('max_discount', $promo->max_discount) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-black focus:border-black" placeholder="50000" step="0.01">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="mb-4">
                            <div class="border-b border-gray-200">
                                <nav class="-mb-px flex space-x-8">
                                    <button type="button" onclick="showProductTab()" id="product-tab" class="py-2 px-1 border-b-2 {{ $promo->product_id ? 'border-black text-black' : 'border-transparent text-gray-500' }} font-medium text-sm">
                                        Pilih Produk
                                    </button>
                                    <button type="button" onclick="showCategoryTab()" id="category-tab" class="py-2 px-1 border-b-2 {{ $promo->category_id ? 'border-black text-black' : 'border-transparent text-gray-500' }} font-medium text-sm hover:text-gray-700 hover:border-gray-300">
                                        Pilih Kategori
                                    </button>
                                </nav>
                            </div>
                        ...
                        <!-- Produk Selection -->
                        <div id="product-selection" class="mt-4 {{ $promo->category_id ? 'hidden' : '' }}">
                            <div class="mb-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 max-h-96 overflow-y-auto p-2 border border-gray-200 rounded-lg">
                                @forelse($products as $product)
                                    <div class="product-card border-2 {{ old('product_id', $promo->product_id) == $product->id ? 'border-black bg-gray-50' : 'border-gray-200' }} rounded-lg p-3 cursor-pointer hover:border-black transition-colors" data-product-id="{{ $product->id }}" data-product-name="{{ $product->name }}">
                                        <input type="radio" name="product_id" id="product_{{ $product->id }}" value="{{ $product->id }}" class="hidden" {{ old('product_id', $promo->product_id) == $product->id ? 'checked' : '' }}>
                        ...
                        <!-- Kategori Selection -->
                        <div id="category-selection" class="mt-4 {{ $promo->product_id ? 'hidden' : '' }}">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                @foreach($categories as $category)
                                    <div class="category-card border-2 {{ old('category_id', $promo->category_id) == $category->id ? 'border-black bg-gray-50' : 'border-gray-200' }} rounded-lg p-4 cursor-pointer hover:border-black transition-colors" data-category-id="{{ $category->id }}">
                                        <input type="radio" name="category_id" id="category_{{ $category->id }}" value="{{ $category->id }}" class="hidden" {{ old('category_id', $promo->category_id) == $category->id ? 'checked' : '' }}>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $promo->start_date->format('Y-m-d')) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-black focus:border-black" required>
                            </div>
                            <div>
                                <label for="end_date" class="block text-sm font-medium text-gray-700 mb-2">Tanggal Berakhir *</label>
                                <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $promo->end_date->format('Y-m-d')) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-black focus:border-black" required>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-8">
                        ...
                        <label class="flex items-center">
                            <input type="checkbox" name="is_active" class="text-black focus:ring-black" {{ old('is_active', $promo->is_active) ? 'checked' : '' }}>
